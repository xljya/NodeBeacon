...
test("owner can sign in and lands on the official dashboard", async ({ page }) => {
...
test("logout returns to the login page", async ({ ownerPage }) => {
  await ownerPage.getByRole("button", { name: "Logout" }).click();
  await ownerPage.waitForURL("**/login");
  await expect(ownerPage.locator(".km-login-card")).toBeVisible();
});
