...
kubectl -n "${NAMESPACE}" cp "${POD}:${REMOTE_DB}" "${WORK_DIR}/nodebeacon.db"

if kubectl -n "${NAMESPACE}" exec "${POD}" -- test -f /data/nodes.yaml; then
  kubectl -n "${NAMESPACE}" cp "${POD}:/data/nodes.yaml" "${WORK_DIR}/nodes.yaml"
fi
