...
node -e 'const v=JSON.parse(process.argv[1]); const expected=Number(process.argv[2]); if(v.summary?.total!==expected||v.summary?.online!==expected) process.exit(1)' "${status_json}" "${EXPECTED_NODES}" || fail "expected ${EXPECTED_NODES} total and online nodes"
